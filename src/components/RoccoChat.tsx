...
export const RoccoChat: React.FC<RoccoChatProps> = ({ missions: externalMissions, onMissionClick }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-[2rem] sm:rounded-[3rem] overflow-hidden border border-zinc-100 shadow-sm">
      {/* Header */}
      <div className="bg-white p-4 sm:p-6 border-b border-zinc-100 flex items-center justify-between">
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[9px] sm:text-[10px] font-bold text-zinc-500 uppercase tracking-widest">{t('rocco.mentor_online')}</span>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden min-h-0">
        ...
        <div className="flex-[3] flex flex-col min-w-0 h-full border-b lg:border-b-0 lg:border-r border-zinc-100 overflow-hidden relative">
              ...
              {messages.map((msg) => (
                ...
                >
                  <div className={cn(
                    ...
                    msg.role === 'user' ? "bg-stormy-teal" : "bg-emerald-action/10"
                  )}>
                    {msg.role === 'user' ? (
                  ...
                  <div className={cn(
                    ...
                    msg.role === 'user' 
                      ...
                      : "bg-white text-zinc-700 border border-zinc-100 rounded-tl-none prose-zinc"
                  )}>
          ...
          {/* Input Area */}
          <div className="shrink-0 p-4 sm:p-6 bg-white border-t border-zinc-100">
            {messages.length === 1 && (
              <div className="flex flex-wrap gap-2 mb-4 overflow-x-auto no-scrollbar pb-2">
                {[
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-white hover:bg-stormy-teal/5 text-zinc-600 hover:text-stormy-teal rounded-xl text-[10px] font-bold border border-zinc-100 hover:border-stormy-teal/20 transition-all flex items-center gap-2 uppercase tracking-widest whitespace-nowrap"
                  >
            ...
            <div className="relative flex items-center gap-2">
              <div className="flex-1 relative">
                <input
                  ...
                  className="w-full pl-4 pr-12 py-4 bg-white border border-zinc-200 rounded-2xl outline-none focus:ring-2 focus:ring-stormy-teal transition-all text-sm font-medium text-zinc-900"
                />
                <button 
                  onClick={() => {
        ...
        <div className="flex-1 bg-white flex flex-col p-4 sm:p-6 space-y-6 lg:space-y-8 overflow-y-auto min-w-0 lg:min-w-[280px] max-h-[35vh] lg:max-h-none shrink-0 border-t lg:border-t-0 lg:border-l border-zinc-100">
          <div>
            <div className="flex items-center gap-2 mb-4">
              <Target className="w-5 h-5 text-stormy-teal" />
              <h3 className="text-sm font-black text-stormy-teal uppercase tracking-tight">{t('rocco.missions_title')}</h3>
            </div>
              ...
              {displayMissions.length > 0 ? (
                displayMissions.slice(0, 3).map((m) => (
                  <button
                    ...
                    className="p-4 bg-white rounded-2xl border border-zinc-100 group hover:border-stormy-teal/20 transition-all cursor-pointer"
                  >
                    <div className="flex items-center gap-3 mb-2">
                      <div className={cn("p-2 rounded-lg shadow-sm text-white", m.color)}>
                        <m.icon className="w-4 h-4" />
                      </div>
                      <span className="text-[10px] font-black text-stormy-teal uppercase tracking-widest">{t(`mission.title_${m.id}`)}</span>
                    </div>
                    <p className="text-[10px] text-zinc-500 font-medium leading-relaxed line-clamp-2">{t(`mission.desc_${m.id}`)}</p>
                    <div className="mt-3 flex items-center justify-end text-stormy-teal opacity-0 group-hover:opacity-100 transition-all">
                 ...
                 <div className="py-10 text-center space-y-3 bg-white rounded-3xl border border-dashed border-zinc-200">
                   ...
                   <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest px-4">
          ...
          <div className="p-6 bg-white rounded-[2rem] border border-zinc-100 text-zinc-900 relative overflow-hidden shrink-0 shadow-sm">
